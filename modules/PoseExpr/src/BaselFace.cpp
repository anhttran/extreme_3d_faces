...
#include "H5Cpp.h"
    ...
    void BaselFace::load(const std::string & model_file)
    {
        try
        {
            ...
            cv::Mat faces_fill_tmp = readH5Dataset(file, "/faces_fill");
            ...
            cv::Mat pair_tmp = readH5Dataset(file, "/pair");
            ...
            // Convert faces_fill to unsigned int
            float* faces_fill_data = (float*)faces_fill_tmp.data;
            int faces_fill_size = faces_fill_tmp.total();
            faces_fill.create(faces_fill_tmp.size(), CV_16U);
            unsigned short* out_faces_fill_data = (unsigned short*)faces_fill.data;
            for (int i = 0; i < faces_fill_size; ++i)
                *out_faces_fill_data++ = (unsigned short)(*faces_fill_data++);
            ...
            // Convert pair to int
            float* pair_data = (float*)pair_tmp.data;
            int pair_size = pair_tmp.total();
            pair.create(pair_tmp.size(), CV_32S);
            int* out_pair_data = (int*)pair.data;
            for (int i = 0; i < pair_size; ++i)
                *out_pair_data++ = (int)(*pair_data++);
        }
