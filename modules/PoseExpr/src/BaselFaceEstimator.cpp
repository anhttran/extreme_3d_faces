...
// Get water-tight triangle connectivity. Output size Fx3
cv::Mat BaselFaceEstimator::getFaces_fill(){
	cv::Mat out;
	bf.faces_fill.convertTo(out, CV_32S);
	return out;
}

// Get symmetry-pair vertex index list
std::vector<int> BaselFaceEstimator::getPair(){
	cv::Mat out;
	bf.pair.convertTo(out, CV_32S);
	std::vector<int> list;
	for (int i=0;i<out.rows;i++)
		list.push_back(out.at<int>(i,0));
	return list;
}
